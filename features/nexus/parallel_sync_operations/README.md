# Nexus sync operations run in parallel
...
- The history contains one scheduled and one completed event per operation, all three
  scheduled by the same workflow task, and no started events, since sync operations never
