...
// `encodingType` above holds its default value, so ProtoJSON omits it. protobufjs 7 nonetheless
// materialized such fields as own properties when decoding, whereas protobufjs 8 leaves them
// absent; `deepEqual` compares own properties, so the two disagree. Compare against an expectation
// that has been through the wire itself, which holds for either version.
