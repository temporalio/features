...
# Cross-function state shared between start() and check_result() (both run on the
# host, sequentially, in this process).
_setup: dict = {}
...
async def start(runner: Runner) -> WorkflowHandle:
    target_id = f"signalwithstart-signal-existing-target-{uuid.uuid4()}"

    # Start the target and leave it running; signal-with-start should signal this
    ...
    return await runner.client.start_workflow(
        CallerWorkflow.run,
        target_id,
        id=f"signalwithstart-signal-existing-caller-{uuid.uuid4()}",
        task_queue=runner.task_queue,
        execution_timeout=timedelta(minutes=1),
    )
